...
class MazeEnv(mujoco_env.MujocoEnv, utils.EzPickle, offline_env.OfflineEnv):
    def __init__(self,
        ...
        self.starts_and_targets = starts_and_targets
        if self.starts_and_targets is None:
            self.reset_locations = list(zip(*np.where(self.maze_arr == EMPTY)))
            self.reset_locations.sort()
        else:
            self.reset_locations = [start for (start, target) in self.starts_and_targets]
        ...
        if self.starts_and_targets is None:
            ...
            self.goal_locations = [target for (start, target) in self.starts_and_targets]
    ...
    def reset_model(self):
        if self.starts_and_targets is None:
            ...
            idx = self.np_random.choice(len(self.starts_and_targets))
